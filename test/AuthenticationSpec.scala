...
class AuthenticationSpec extends Specification{
  ...
  "Authentication" should {
    ...
    "Successfully Logged In" in new WithApplication{
      val login= route(FakeRequest(POST,"/submit").withFormUrlEncodedBody("email"->"Rahul@knoldus.com","password"->"1234","confirmPassword"->"1234")).get
      status (login) must equalTo(303)
    }
    ...
    "render email page with logged in" in new WithApplication{
      val login= route(FakeRequest(GET,"/changeEmail").withSession("email"->"Rahul@knoldus.com","password"->"1234")).get
      status (login) must equalTo(OK)
    }

    "render email submit page with logged in" in new WithApplication{
      val login= route(FakeRequest(GET,"/changeEmailSubmit").withSession("email"->"Rahul@knoldus.com","password"->"1234")).get
